		 ...
		 <input type="hidden" class="form-control dy_wage" value="{{$wage}}"  required>
            <form id="createForm" method="post" action="" enctype="multipart/form-data"> 
                ...
                <div class="card">
                            ...
                            <div class="tab-pane fade show active" id="TH" aria-labelledby="home-tab">
                                <div class="row"> 
                                    ...
                                    <div class="form-group col-lg-3">
                                        ...
                                        <b>Quantity :</b> <span id="quantity">{{ $row->balance }}</span> <br>
                                        <b>Unit :</b> <span id="unitPO">{{ $row->name_th.'/'.$row->name_en }}</span> <br>
                                        
                                        <b>Blue crate :</b> <span id="blue_crate">{{ $row->blue_crate }}</span>
                                        <input type="hidden" id="sorting_id" name="sorting_id" value="{{ $row->id }}">
                                        <input type="hidden" id="balance" name="balance" value="{{ $row->balance }}">
                                        <input type="hidden" id="cost_asl" name="cost_asl" value="{{ $row->total_price }}">
                                        <input type="hidden" id="avg_weight" name="avg_weight" value="{{ $row->avg_weight }}">
                                      @if($row->isBox=='no')
                                    <!--  <input type="hidden" id="aslPrice" name="aslPrice" value="{{ $row->total_price}}"> -->
                                     <input type="hidden" id="aslPrice" name="aslPrice" value="{{ $row->total_price}}"> 
                                     @else

                                        <input type="hidden" id="aslPrice" name="aslPrice" value="{{ $row->total_price}}">
                                      <!--   <input type="hidden" id="aslPrice" name="aslPrice" value="{{ $row->receiving_cost}}"> -->
                                        @endif

                                        <input type="hidden" id="item" name="item" value="{{ $row->item }}">
                                        <input type="hidden" id="unit" name="unit" value="{{ $row->unit }}">
                                    ...
                                    <div class="form-group col-lg-3">
                                        ...
                                        <input type="hidden" class="form-control" id="ean_cost" name="ean_cost" placeholder="Auto Calculate" readonly>
                                    ...
                                    <tr id="row1" data-id="1">
                                        <td data-label="EAN">
                                            <select name="ean[]" id="ean1" class="form-control select_ean select2" style="width:100%" required>
                                                ...
                                                @php
                                                    $eans = DB::table('products')->leftJoin('setups','products.id','=','setups.product')->select('products.id','products.name')
                                                    ->where(['setups.item'=>$row->item,'setups.type'=>'item','products.status'=>'on'])->get();
                                                @endphp
                                                @foreach ($eans as $ean)
                        ...
                        <div class="row">
                            <div class="header">
                                <div class="col-lg-12">
                                    ...
                                    <button type="button" class="btn btn-xs btn-success add-row" data-id="{{ $row->item }}" style="display:">add</button>
                                </div>
                            </div>
                        </div>
                   ...
                   <!--  <input type="hidden" id="balanceENA" value="0"> -->
