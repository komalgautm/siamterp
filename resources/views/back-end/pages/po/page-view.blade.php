            ...
            <form id="viewForm" method="post" action="" enctype="multipart/form-data">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-lg-3" style="display:">
                                ...
                                <span><b>Invoice Number :</b> {{ $row->invoice_no }}</span> <br>
                                <span><b>Invoice date :</b>@if($row->invoice_date != '') {{ date('d-M-Y',strtotime($row->invoice_date)) }} @endif</span> <br>
                                <span style="float:right;"><button type="button" class="btn btn-success" data-toggle="modal" data-target="#dn" > Invoice details </button></span>
                            ...
                            <table class="table table-striped no-footer table-res" id="sorted_table" style="border-collapse: collapse !important" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="Type">
                                            <!-- {{ $item->type }} -->
                                            @if(strtolower($item->type)=='item') Produce @endif
                                        </td>
                                        <td data-label="Name">
                        ...
                        <div class="row">
                            <div class="col-lg-3">
                                <select class="form-control" name="status" id="status" @if($row->status == 'receive' || $row->status == 'cancel') disabled @endif>
                    ...
                    <div class="card-footer">
                        <div class="row">
                            @if($row->status != 'receive' && $row->status != 'cancel')
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Invoice Details (PO: <b class="poCode">{{$row->code}}</b>)</h4>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
            </div>
            <form action="{{url('po/invoice_detail_update')}}" method="post">
            <input type="hidden" name="id" id="poid" value="{{$row->id}}">
              @csrf
            <div class="modal-body">
                <div class="row">
                    <div class="form-group col-lg-12">
                        <h6>Invoice Number</h6>
                       <input type="text" name="invoice_no" id="invoice_no" class="form-control" value="{{$row->invoice_no}}" required>
                    </div>
                ...
                <div class="row">
                    <div class="form-group col-lg-12">
                        <h6>Invoice Date</h6>
                       <input type="date" name="invoice_date" id="invoice_date" class="form-control" value="{{$row->invoice_date}}"  required>
                    </div>
