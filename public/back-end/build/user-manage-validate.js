$('#signupForm').validate({
    ...
    rules:{
        ...
        username: { 
            required: true, 
            // email: true ,
            remote: { url:window.location.origin+'/TERP/user/exist', type:"post", data:{ _token:$('input[name="_token"]').val() }, }
        },
...
$('#resetForm').validate({
    ...
    rules:{
        username: { 
            ...
            // email: true,
            remote: { url: window.location.origin+'/TERP/user/exist-on-reset', type:"get" }
        },
        password: { required:true, minlength:6 },
        confirm_password: { required:true, minlength:6, equalTo:"#password" },
    },
...
$(document).on('click','.deleteItem',function(){
    ...
    Swal.fire({
        ...
        $.ajax({type:'post',url:window.location.origin+'/TERP//users/destroy',data:{'id[]':id,_method:'DELETE',_token:$('input[name="_token"]').val()},
